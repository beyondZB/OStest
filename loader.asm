...
%else
	org  0000h			;实际中嵌入扇区的代码会被加载到9000:0
%endif
...
%else
BaseOfStack		equ	0000h	; Boot状态下堆栈基地址(栈底, 从这个位置向低地址生长)
%endif

BaseOfKernel		equ	8000h	; KERNEL.BIN 被加载到的位置 ----  段地址
OffsetOfKernel 		equ	0000h	; KERNEL.BIN 被加载到的位置 ---- 偏移地址
...
; *****************************************************************************************************
	jmp	BaseOfKernel:OffsetOfKernel	; 这一句正式跳转到已加载到内存中的 KERNEL.BIN 的开始处
						; 开始执行 KERNEL.BIN 的代码
